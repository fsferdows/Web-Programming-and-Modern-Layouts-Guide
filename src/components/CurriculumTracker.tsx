...
export default function CurriculumTracker() {
  ...
  const getMasteryLevel = (pct: number) => {
    if (pct === 0) return { label: "Initiate", color: "bg-slate-100 text-slate-700 border-slate-200" };
    if (pct <= 20) return { label: "Apprentice", color: "bg-indigo-50 text-indigo-700 border-indigo-100" };
    if (pct <= 60) return { label: "Competent Web Developer", color: "bg-blue-50 text-blue-700 border-blue-100" };
    if (pct <= 80) return { label: "Senior Engineer in Training", color: "bg-amber-50 text-amber-700 border-amber-100" };
    return { label: "Master of Web Pillars", color: "bg-emerald-50 text-emerald-700 border-emerald-100" };
  };

  const mastery = getMasteryLevel(percentage);

  return (
    <div className="bg-white rounded-2xl border border-slate-200/90 shadow-2xs overflow-hidden" id="curriculum-tracker-box">
      {/* Tracker Header */}
      <div className="p-5 border-b border-slate-100 bg-slate-50/75 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border ${mastery.color}`}>
          {percentage === 100 ? <Trophy className="w-3.5 h-3.5" /> : <Award className="w-3.5 h-3.5" />}
      ...
      <div className="p-5 grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
        <div className="md:col-span-3 flex flex-col items-center justify-center text-center p-3 bg-slate-50/50 rounded-xl border border-slate-100">
          <span className="text-3xl font-extrabold text-slate-950 font-mono tracking-tight">
            {percentage}%
          </span>
          <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider font-mono mt-1">
            Mastery level
          </span>
          <span className="text-xs text-slate-600 mt-1.5">
        ...
        <div className="md:col-span-9 space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center text-xs">
              <span className="font-semibold text-slate-600 font-sans">Syllabus Progress</span>
              <span className="font-bold text-slate-800 font-mono">{percentage}% Complete</span>
            </div>
            ...
            {CURRICULUM_SECTIONS.map((section) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`text-[9px] font-mono shrink-0 ml-1.5 ${isCompleted ? "text-amber-300" : "text-slate-400"}`}>
                    0{section.number}
                  </span>
                </button>
              );
